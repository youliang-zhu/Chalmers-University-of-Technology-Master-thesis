...
faithfulness, outcome usefulness, and length. However, the current code
artifact does not yet support all four as a synchronized, executable reward
contract. The parser schema exposes event fields for \texttt{session\_id},
\texttt{session\_time}, \texttt{event\_time}, and text, while the faithfulness
reward path still expects the older \texttt{inferred} attribute when selecting
direct events. Consequently, this chapter treats source faithfulness as a
method requirement and planned reward component, not as a verified active
component for reported GRPO training unless the implementation is first
synchronized. \textcolor{red}{TODO: Before reporting GRPO training results,
either update the reward path so faithfulness checks all parsed source-grounded
events under the current schema, or report the run as using only the executable
format, outcome, and length components.}
...
The intended full reward, after the faithfulness path is synchronized, is
\[
  R(\tau)=
...
is the executable reward subset:
\[
  R_{\mathrm{exec}}(\tau)=
  w_f R_{\mathrm{fmt}}(\tau)
  + w_o R_{\mathrm{out}}(\tau)
...
default. The reward boundary is less settled. Format, outcome, and length are
...
implementation or evaluated diagnostically. Chapter 4 evaluates whether these
design choices improve answer quality, temporal grounding, compactness, and
hidden-dependency retrieval on public memory benchmarks and ChainMem.
