...
generation. The design separates four desired signals: format validity, source
faithfulness, outcome usefulness, and length. \textcolor{red}{NOTSURE: The
final thesis reward contract remains unresolved at this escalation point. The
current text cannot yet make a single defensible claim about whether reported
GRPO training optimizes format/outcome/length only, or a synchronized
format/faithfulness/outcome/length objective. This must be decided from the
final implementation and experiment records before Chapter~4 reports GRPO
results.} The current code artifact does not yet support all four as a
synchronized, executable reward contract. The parser schema exposes event
fields for \texttt{session\_id}, \texttt{session\_time},
\texttt{event\_time}, and text, while the faithfulness reward path still
expects the older \texttt{inferred} attribute when selecting direct events.
Consequently, this chapter treats source faithfulness as a method requirement
and planned reward component, not as a verified active component for reported
GRPO training unless the implementation is first synchronized.
\textcolor{red}{TODO: Before reporting GRPO training results, choose one final
reward version and make Methods and Results use only that version: either
update the reward path so faithfulness checks parsed source-grounded events
under the current schema and report its active weight, or report the run as
using only the executable format, outcome, and length components and move
faithfulness/verdict supervision to diagnostics or limitations.}
...
\textbf{Source faithfulness requirement.} Each emitted event is supposed to be
directly supported by its cited session. This requirement is needed because a
compact evidence pool is useful only if compression preserves attribution.
Under the current parser schema, every parsed event should be treated as a
source-grounded claim: evidence that depends on cross-session inference should
be expressed through separate cited events and the \texttt{reasoning} field
rather than as a single unsupported event. The intended faithfulness reward
would check event text against the corresponding session with a frozen verifier
and reward the proportion of supported events. In the present artifact, though,
that reward path has not been reconciled with the current parser because it
still filters on the removed \texttt{inferred} field. It should therefore be
used only after the schema mismatch is fixed and recorded.
...
The implemented length term is a penalty,
$R_{\mathrm{len}}=-\min(1,\mathrm{tokens}/2048)$, so adding it with a positive
weight encourages shorter Synthesizer outputs. It must not dominate outcome or
verdict quality, because a trivially short but uninformative evidence pool is
not useful memory.

The unresolved planned full reward, after the faithfulness path is synchronized,
would be
\[
  R(\tau)=
...
For any result reported before that synchronization, the faithful description
would instead be the executable reward subset:
\[
  R_{\mathrm{exec}}(\tau)=
  w_f R_{\mathrm{fmt}}(\tau)
  + w_o R_{\mathrm{out}}(\tau)
  + w_l R_{\mathrm{len}}(\tau),
\]
with source faithfulness evaluated, at most, as a post-hoc diagnostic rather
than as an optimization signal. This constraint is important for Chapter 4:
claims about learned source faithfulness or a faithfulness--compactness
trade-off require a run whose reward implementation and parser schema are
aligned. \textcolor{red}{NOTSURE: These two formulas are left here only to
expose the unresolved method fork. The final thesis should keep exactly one
reported reward objective and delete the alternative before submission.}
...
\begin{figure}[H]
  ...
  \fbox{\begin{minipage}{0.92\textwidth}
    ...
    signal figure. It should mark format on the Synthesizer artifact, outcome
    at terminal answer evaluation, compactness as a token penalty, the verdict
    as the terminal gate, and faithfulness as a planned/source-support check
    that requires schema synchronization before it can be claimed as active.}
    \vspace{0.8em}
  \end{minipage}}
  \caption{Reward signals in a NanoMem trajectory. Format supervises the
  structured evidence artifact, outcome and length connect terminal evidence to
  downstream usefulness and compactness, and the verdict gates whether terminal
  outcome evaluation is reached. Source faithfulness is a method requirement,
  but it should be claimed as an active reward only after its implementation is
  synchronized with the parser schema.}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.18\textwidth}p{0.21\textwidth}p{0.19\textwidth}p{0.23\textwidth}p{0.09\textwidth}}
    ...
    Faithfulness & Direct support between event text and cited session & Planned reward or post-hoc diagnostic until schema is synchronized & Unsupported compressed evidence & Current reward path still expects removed \texttt{inferred} field \\
...
default. \textcolor{red}{TODO: Resolve the reward boundary before writing
Chapter~4. The implementation boundary must name the exact active reward
objective used by the reported GRPO run, including whether faithfulness was
fixed and enabled or excluded from optimization.} The reward boundary is less
settled. Format, outcome, and length are
traceable in the current reward pipeline, with length implemented as a negative
token-ratio penalty. The source-faithfulness reward is not yet a reliable
active method claim because the parser no longer exposes the older
\texttt{inferred} attribute while the reward code still reads that field. A
reported training run must therefore either document a synchronized
faithfulness implementation or exclude faithfulness from the claimed
optimization objective. Chapter 4 should still report run-specific model
identities, data splits, group size, checkpoint selection, and any
configuration overrides, and it must align its GRPO claims with whichever
reward path was actually executable.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.22\textwidth}p{0.27\textwidth}p{0.20\textwidth}p{0.21\textwidth}}
    ...
    Reward pipeline & \texttt{training/grpo/trainer/rewards.py}; reward weights config & XML and terminal answer to scalar reward & Format/outcome/length executable; faithfulness requires schema synchronization before active-use claims \\
...
sufficiency-aware evidence construction, while source faithfulness remains a
required evidence property that must be either synchronized in the reward
implementation or evaluated diagnostically. \textcolor{red}{NOTSURE: The
Chapter~4 interface is not final until the reward contract is fixed; Results
must not claim learned source faithfulness, verdict correctness, or a reward
ablation unless the corresponding active objective is verified from the final
training run.} Chapter 4 evaluates whether these design choices improve answer
quality, temporal grounding, compactness, and hidden-dependency retrieval on
public memory benchmarks and ChainMem.
